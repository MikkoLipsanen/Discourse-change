...
parser = argparse.ArgumentParser(description='Supervised pattern based classification')
...
parser.add_argument('--save_model_path', type=str, default='model/supervised_pattern_model.pt', help='path to save the model')
parser.add_argument('--batch_size', type=int, default=300, help='defines batch size')
...
parser.add_argument('--job_name', type=str, default='supervised_pattern', help='name used in the saved result files')
parser.add_argument('--n_cats', type=int, default=7, help='number of discourse patterns in data')
...
class ClassificationDataset(Dataset):
    ...
    def get_data(self):
        ...
        data = data_dict['data']
        pattern_label = data_dict['pattern_label'] - 1
...
cl = Classifier(args.rnn_output_size, args.n_cats).to(device)
...
def eval_results(cat_preds, y_cat):
    correct = (cat_preds == y_cat).sum().item()
    prec_cat, rec_cat, f_cat, s_cat = precision_recall_fscore_support(y_cat.detach().numpy(), cat_preds.detach().numpy(), average='weighted', zero_division=0)
    rand = adjusted_rand_score(y_cat.detach().numpy(), cat_preds.detach().numpy())
    
    return correct, prec_cat, rec_cat, f_cat, rand
...
def train_model():
    ...
    for (x_train, y_train) in train_dataloader:
        ...
        y_cat = torch.tensor(train_data.batch_pattern_labels, dtype = torch.long)
        ...
        _, cat_preds = torch.max(cl_output, dim=1)
        
        # Calculate training loss
        tr_loss = criterion(cl_output, y_cat)
        loss += tr_loss.item()
        ...
        corr, prec_cat, rec_cat, f_cat, rand = eval_results(cat_preds, y_cat)
    ...
    cat_accuracy = correct / args.tr_size

    mean_prec_cat = precision / num_batches
    mean_rec_cat = recall / num_batches
    mean_f_cat = f_score / num_batches
    mean_rand = rand_score / num_batches

    return train_loss, cat_accuracy, mean_prec_cat, mean_rec_cat, mean_f_cat, mean_rand
...
def eval_model():
    ...
    with torch.no_grad():
        ...
        for (x_val, y_val) in validation_dataloader:
            ...
            y_cat = torch.tensor(validation_data.batch_pattern_labels, dtype = torch.long)
 
            # Reset batch labels between batches
            validation_data.reset_batch_labels()
            ...
            val_loss = criterion(cl_output, y_cat)
            loss += val_loss.item()
        ...
        cat_accuracy = correct / args.val_size
    ...
    return validation_loss, cat_accuracy, mean_prec_cat, mean_rec_cat, mean_f_cat, mean_rand
...
def test_model():
    ...
    with torch.no_grad():
        ...
        for (x_test, y_test) in test_dataloader:
            ...
            y_cat = torch.tensor(test_data.batch_pattern_labels, dtype = torch.long)
        
            # Reset batch labels between batches
            test_data.reset_batch_labels()
            ...
            # Calculate loss for test data
            test_loss = criterion(cl_output, y_cat)
            loss += test_loss.item()
    ...
    return test_loss, cat_accuracy, mean_prec_cat, mean_rec_cat, mean_f_cat, mean_rand
...
print("Test loss : %.3f | Test accuracy %.3f | Test F-score %.3f | Test rand score %.3f" %(test_loss, test_accuracy, test_mean_f, test_rand))
...
file_to_write = open(args.res_path + 'res_dicts/' + args.job_name + '_supervised_results.pkl', 'wb')
