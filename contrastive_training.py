...
parser = argparse.ArgumentParser(description='NN For Contrastive Pretraining')
...
parser.add_argument('--num_timepoints', type=int, default=100, help='defines number of timepoints in sample set')
parser.add_argument('--tr_size', type=int, default=3000, help='defines the size of the training dataset')
parser.add_argument('--val_size', type=int, default=2000, help='defines the size of the validation dataset')
...
parser.add_argument('--epochs', type=int, default=50, help='defines the number of epochs')
...
parser.add_argument('--rnn_output_size', type=int, default=300, help='Size of the RNN output vector')
...
parser.add_argument('--distance_measure', type=str, default='cosine', help='Distance used in contrastive loss calculation')
parser.add_argument('--tau', type=float, default=0.6, help='Temperature parameter for loss functions')
parser.add_argument('--job_name', type=str, default='contrastive_pretraining', help='name used in the saved result files')
...
class ContrDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index >= self.ind:
            self.data, self.label = self.get_data()
            self.batch_labels.append(self.label)
...
train_data = ContrDataset(tr_sets, max_docs, len(tr_sets), 'train')
validation_data = ContrDataset(val_sets, max_docs, len(val_sets), 'train')
...
criterion = losses.SupConLoss(temperature=args.tau)
...
def train_model():
    ...
    for x_train in train_dataloader:
        ...
        contr_loss = criterion(rnn_output, torch.tensor(train_data.batch_labels))
...
def eval_model():
    ...
    with torch.no_grad():
        ...
        for x_val in validation_dataloader:
            ...
            # Calculate contrastive loss
            contr_loss = criterion(rnn_output, torch.tensor(validation_data.batch_labels))
...
def test_model():
    ...
    losses = []
    output_vecs = np.zeros((args.ts_size,args.rnn_output_size))
    labels = []
    ind = 0
    ...
    with torch.no_grad():
        ...
        for x_test in test_dataloader:
            ...
            res_array = rnn_output.detach().cpu().numpy()
            n_vecs = res_array.shape[0]
            output_vecs[ind:ind+n_vecs,:] = res_array
            ind += n_vecs

            labels += test_data.batch_labels
        
            # Calculate contrastive loss
            contr_loss = criterion(rnn_output, torch.tensor(test_data.batch_labels))
    ...
    return losses, output_vecs, labels
...
for epoch in range(epoch_count+1, args.epochs+1): 
    ...
    with open(args.res_path + args.job_name + '_epoch%i.txt' % epoch, 'w') as f:
        f.write(epoch_res)
...
test_loss, output_vecs, labels = test_model()

print("Test loss : %.3f" %np.mean(test_loss))

test_results = {'data': output_vecs, 'labels': labels, 'args': args}

file_to_write = open(args.res_path + 'sample_vecs/'+ args.job_name + '_vecs.pkl', 'wb')
pickle.dump(test_results, file_to_write)

##########Save the results#################

results = {'tr_loss': tr_losses, 'val_loss': val_losses, 'args': args}

# Save loss and accuracy
file_to_write = open(args.res_path + 'res_dicts/' + args.job_name + '_results.pkl', 'wb')
